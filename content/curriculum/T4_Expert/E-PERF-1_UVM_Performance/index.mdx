...
import { EventSchedulerVisualizer } from '@/components/visualizers/EventSchedulerVisualizer';
...
### IEEE 1800-2023 Clause 4: The Event Scheduler
Performance bottlenecks often trace back to a poor understanding of the SystemVerilog scheduler. Minimizing context switches across the Active, Inactive, NBA, and Postponed regions is key to scaling regression throughput.

<EventSchedulerVisualizer />

- **Active Region:** Evaluates blocking assignments (`=`) and continuous assignments.
- **NBA Region:** Updates variables cleanly sequenced via non-blocking assignments (`<=`).
- **Postponed Region:** Executes sampling after all signals settle, preventing race conditions.

> **Interview Pitfall:** Mixing `=` and `<=` indiscriminately causes the scheduler to "thrash" between Active and NBA regions for the same timestamp, stalling the simulator in an infinite delta-cycle loop (a zero-time hang).
