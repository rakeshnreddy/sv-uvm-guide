...
1.  **Driver waits:** `seq_item_port.get_next_item(req)` blocks until a sequence supplies work.
2.  **Sequence requests & delivers:** `start_item()` wins arbitration (`wait_for_grant`) and `finish_item()` hands the item to the sequencer (the `uvm_do` macros wrap these calls).
3.  **Driver receives:** The sequencer releases the item, letting `get_next_item()` return so the driver can `drive_transfer(req)` on the interface.
4.  **Driver finishes:** `seq_item_port.item_done(rsp)` signals completion (and optionally returns a response), freeing the sequencer to schedule the next request.
