...
<InteractiveCode
  ...
  fileName="ral_block.sv"
  code={`class status_reg extends uvm_reg;
...
endclass`}
  explanationSteps={[
    { target: "1-8", title: "Model the register", explanation: "Creates a typed view of the DUT register with access policy." },
    { target: "10-19", title: "Map it", explanation: "Place registers into an address map so bus agents can locate them." },
    { target: "21-28", title: "Use both doors", explanation: "Sequences choose frontdoor for protocol accuracy or backdoor for speed/reset." }
  ]}
/>
...
<InteractiveCode
  language="systemverilog"
  fileName="powerup_virtual_seq.sv"
  code={`class powerup_vseq extends uvm_sequence;
...
endclass`}
  explanationSteps={[
    { target: "4-5", title: "Handles via virtual sequencer", explanation: "Sequencer handles are injected from the environment so the virtual sequence stays generic." },
    { target: "9-11", title: "Stage 1: control", explanation: "Configure control logic before fabric traffic starts." },
    { target: "13-22", title: "Concurrent stage", explanation: "Run bring-up and monitoring in parallel—no `#delays` required." }
  ]}
/>
