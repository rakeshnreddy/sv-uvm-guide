...
>
  ...
  Real silicon fires interrupts at awkward times. Convert those asynchronous edges into structured UVM transactions so sequences can acknowledge, recover, and resume deterministically—no shotgun `#delays`, no lost handshakes.

  ## Event-Driven Interrupt Flow

  <InteractiveCode
    language="systemverilog"
    fileName="interrupt_flow.sv"
    code={`
...
endclass
    `}
    explanationSteps={[
      { target: "1-20", title: "Monitor detects interrupts", explanation: "Translate pin wiggles into typed transactions and trigger an event so multiple listeners can react." },
      { target: "22-30", title: "Router policy", explanation: "Subscriber queues interrupts—swap in priority queues or throttling logic without touching sequences." },
      { target: "32-64", title: "Virtual sequence reaction", explanation: "Virtual sequence waits on the mailbox and launches clear/replay sequences on configured agent sequencers." }
    ]}
  />
  ...
  <Quiz questions={[
    {
      ...
      explanation: "A queue centralizes prioritization, coalescing, and rate limiting so the recovery logic stays deterministic." }
    },
    {
      ...
      explanation: "Virtual sequences coordinate which agent should acknowledge, using handles stored on the virtual sequencer." }
    }
  ]} />
