...
**Why it works**
- Lines 6-11 apply a global type override so every `alu_driver` morphs into the error-injecting variant.
- Lines 13-19 scope an override to a single hierarchical path—only the DMA agent swaps drivers.
- Lines 21-25 instantiate the environment and immediately print factory state to confirm the overrides fired.
