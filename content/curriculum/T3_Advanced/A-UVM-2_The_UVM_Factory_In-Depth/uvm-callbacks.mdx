...
>
  ...
  <InteractiveCode
    ...
    fileName="driver_with_callbacks.sv"
    code={`
...
endclass
    `}
    explanationSteps={[
      { target: "1-18", title: "Callback base & specializations", explanation: "Define a base callback with hooks, then extend for specific behaviours." },
      { target: "20-35", title: "Driver integration", explanation: "Use `uvm_callbacks::pre_drive`/`post_drive` to invoke registered callbacks." },
      { target: "37-44", title: "Registration", explanation: "Environment registers callbacks on the driver instance—no source edits required." }
    ]}
  />
  ...
  <Quiz questions={[
    {
      question: "Why prefer callbacks over subclassing for logging?",
      answers: [
        { text: "They execute faster", correct: false },
        { text: "They add behaviour without needing a new driver subclass", correct: true },
        { text: "They replace the factory", correct: false },
        { text: "They only work with monitors", correct: false }
      ],
      explanation: "Callbacks decouple instrumentation from the component implementation so multiple observers can coexist." }
    },
    {
      question: "How do you invoke registered callbacks?",
      answers: [
        { text: "Call `uvm_callbacks#(...).do()`", correct: false },
        { text: "Use the typedef helper, e.g., `my_driver_cbs_t::pre_drive(...)`", correct: true },
        { text: "Callbacks fire automatically", correct: false },
        { text: "`factory.print()`", correct: false }
      ],
      explanation: "The typedef provides static helper methods that iterate all registered callbacks." }
    }
  ]} />
