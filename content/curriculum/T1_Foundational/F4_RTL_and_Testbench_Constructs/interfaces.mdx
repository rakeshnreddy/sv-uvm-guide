...
<div className="my-6 rounded-2xl border border-muted bg-muted/40 p-6">
  <h3 className="text-lg font-semibold">Signal Timing Snapshot</h3>
  <p className="mt-2 text-sm text-muted-foreground">
    Explore the interactive version inside the Practice &gt; Visualizations area. Below is the
    quick reference legend we surface there:
  </p>
  <ul className="mt-4 grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
    <li><strong>Sample markers</strong> (green) show when monitors capture interface traffic.</li>
    <li><strong>Drive markers</strong> (blue) highlight the exact edges drivers assert bus values.</li>
    <li><strong>Clock ribbon</strong> animates through the handshake to reinforce phase ordering.</li>
    <li><strong>Parameter badges</strong> remind you which widths and modport views power the run.</li>
  </ul>
</div>
