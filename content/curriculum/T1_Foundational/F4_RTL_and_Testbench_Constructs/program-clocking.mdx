...
<div className="my-6 rounded-2xl border border-muted bg-muted/40 p-6">
  <h3 className="text-lg font-semibold">Timing Heuristics</h3>
  <p className="mt-2 text-sm text-muted-foreground">
    Picture the handshake in three phases:
  </p>
  <ol className="mt-4 list-decimal space-y-2 pl-5 text-sm text-muted-foreground">
    <li>
      <strong>Sample window.</strong> The clocking block captures DUT outputs a whisker ahead of the
      clock edge, so monitors see the final, settled value for the cycle that just completed.
    </li>
    <li>
      <strong>Active edge.</strong> On the `posedge` the DUT updates its flops; program blocks stay in
      the reactive region, so they observe the post-edge state.
    </li>
    <li>
      <strong>Drive skew.</strong> Testbench drivers assert new requests a defined delay after the
      edge, guaranteeing the DUT samples clean data on the following cycle.
    </li>
  </ol>
  <p className="mt-3 text-sm text-muted-foreground">
    Revisit the full animation under <code>Practice → Visualizations → Interface Signal Flow</code>
    whenever you need the moving timeline.
  </p>
</div>
