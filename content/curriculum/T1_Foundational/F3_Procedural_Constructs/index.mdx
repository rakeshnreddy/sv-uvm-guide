...
### Common Pitfalls
- Mixing blocking and non-blocking assignments to the same signal, causing races.
- Using `always` instead of `always_ff`/`always_comb`, hiding missing signals in sensitivity lists.
- Forgetting to terminate `fork/join` threads, leaving simulations hanging.
