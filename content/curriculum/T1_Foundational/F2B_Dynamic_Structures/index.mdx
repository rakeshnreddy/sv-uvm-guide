...
- **Packed Cube Index Order** — the new scenario instantiates `logic [P1-1:0][P2-1:0][P3-1:0] my_array [U1][U2][U3];` with small numeric ranges so you can watch every `u` index fall on the left and every `p` index stay on the right. The memory walk animates entries such as `my_array[u1=0][u2=1][u3=0][p1=1][p2=0][p3=1]` so you see the packed bits flip faster than the unpacked coordinates.
