...
- **The Analogy:** An interface is like a USB cable—it bundles power, ground, and data lines. The modport is the connector type (Type-A vs. Type-B) that ensures you plug it in correctly.
...
## The Bridge to Classes: Virtual Interfaces
_How dynamic objects talk to static hardware._

In modern verification (UVM), your testbench drivers are **classes** (dynamic objects created at runtime). But your DUT and interfaces are **modules** (static objects created at compile time). A class cannot "contain" a module instance.

Enter the **Virtual Interface**. It is a handle (pointer) that lets a class reference a static interface instance.

```mermaid
graph LR
    subgraph Static World [Static World (Hardware)]
        DUT[DUT Module]
        IF[Interface Instance]
        DUT -- ports --> IF
    end

    subgraph Dynamic World [Dynamic World (Classes)]
        Driver[Driver Class]
    ...
    VIF -. points to .-> IF
    style VIF stroke-dasharray: 5 5
```
...
class driver;
  ...
  task run();
    forever begin
      // 2. Drive signals via the handle
      @(posedge vif.clk);
      vif.valid <= 1;
    end
  endtask
...
## Push Further
- Learn how to parameterize an interface so you can instantiate an 8-bit or 32-bit bus without duplicating code.
- Investigate what happens when you use an interface array to connect multiple identical slaves (like memory banks) to an interconnect.
  ...
  <CardContent className="space-y-4">
    <details className="group rounded-lg bg-slate-800 p-4 open:bg-slate-800/80">
      ...
      <div className="mt-4 text-sm text-slate-300 leading-relaxed">
        <p>A <strong>virtual interface</strong> is a variable that points to an actual interface instance. Since classes (like UVM drivers) are dynamic and cannot contain static hierarchy, they use virtual interfaces to communicate with the static DUT. You pass the handle from the `module` world to the `class` world via the `config_db`.</p>
      </div>
    </details>

    <details className="group rounded-lg bg-slate-800 p-4 open:bg-slate-800/80">
      <summary className="flex cursor-pointer items-center justify-between font-medium text-slate-200">
        Can an interface have parameters?
        <span className="text-slate-400 transition group-open:rotate-180">▼</span>
      </summary>
      <div className="mt-4 text-sm text-slate-300 leading-relaxed">
        <p>Yes. Interfaces can be parameterized just like modules. This is essential for creating reusable verification IP (VIP) that can handle different bus widths or configurations.</p>
      </div>
    </details>
  </CardContent>
</Card>
