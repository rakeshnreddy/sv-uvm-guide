...
flashcards: "F2C_Procedural_Constructs"
---
...
- **Zombie Threads:** Forgetting to terminate `fork/join_none` threads leaves them running forever, potentially hanging the simulation.
- **The \`disable fork\` Trap:** Calling \`disable fork;\` kills *all* active child processes spawned by the current process, which can accidentally kill unrelated threads if not scoped properly. Always wrap targeted forks in a named \`begin...end\` block and disable the block name, or use an isolated process scope.

## The Forbidden Fruit: `always_latch`
_When you actually want a latch (which is almost never)._

SystemVerilog provides `always_latch` to model transparent latches. Unlike flip-flops (`always_ff`), latches pass data through when the enable is high and hold it when low.

```systemverilog
always_latch begin
  if (enable)
    q <= d;
  // No 'else' branch! This infers a latch.
end
```

> [!CAUTION]
> **Avoid Latches**
> In 99% of digital design, latches are accidental (due to incomplete `if/case` in combinational logic) and cause timing headaches. Only use `always_latch` if you are explicitly designing a clock gating cell or a low-power state retention block. If you see a latch inferred from `always_comb`, it is a bug.
  ...
  <CardContent className="space-y-4">
    <details className="group rounded-lg bg-slate-800 p-4 open:bg-slate-800/80">
      <summary className="flex cursor-pointer items-center justify-between font-medium text-slate-200">
        What is a race condition in Verilog/SystemVerilog?
        <span className="text-slate-400 transition group-open:rotate-180">▼</span>
      </summary>
      <div className="mt-4 text-sm text-slate-300 leading-relaxed">
        <p>A race condition occurs when the simulation result depends on the arbitrary order in which the simulator executes events at the same timestamp. A classic example is two `always` blocks reading and writing the same variable using blocking assignments (`=`) at the same clock edge.</p>
      </div>
    </details>
    ...
    <details className="group rounded-lg bg-slate-800 p-4 open:bg-slate-800/80">
      ...
      <div className="mt-4 text-sm text-slate-300 leading-relaxed">
        <p>Non-blocking assignments schedule the update for the NBA region, ensuring that all `always_ff` blocks sample the "old" value of their inputs before any outputs change. This mimics the behavior of physical flip-flops and prevents hold-time races in simulation (IEEE 1800-2023 §10.4.2).</p>
      </div>
    </details>

    <details className="group rounded-lg bg-slate-800 p-4 open:bg-slate-800/80">
      <summary className="flex cursor-pointer items-center justify-between font-medium text-slate-200">
        Explain the difference between fork-join, fork-join_any, and fork-join_none.
        <span className="text-slate-400 transition group-open:rotate-180">▼</span>
      </summary>
      <div className="mt-4 text-sm text-slate-300 leading-relaxed">
        <p><strong>fork-join:</strong> The parent process blocks until ALL child processes finish.</p>
        <p><strong>fork-join_any:</strong> The parent process blocks until ANY ONE child process finishes. The other children continue running in the background.</p>
        <p><strong>fork-join_none:</strong> The parent process does not block at all. All child processes run in the background concurrently. (IEEE 1800-2023 §9.3.2)</p>
      </div>
    </details>

    <details className="group rounded-lg bg-slate-800 p-4 open:bg-slate-800/80">
      <summary className="flex cursor-pointer items-center justify-between font-medium text-slate-200">
        What is the danger of using `disable fork;` and how do you avoid it?
        <span className="text-slate-400 transition group-open:rotate-180">▼</span>
      </summary>
      <div className="mt-4 text-sm text-slate-300 leading-relaxed">
        <p>`disable fork;` terminates all active descendants of the calling process. If your task spawns a background monitor, then later uses `fork-join_any` for a timeout and calls `disable fork;` to clean up the timeout threads, it will unintentionally kill the monitor too.</p>
        <p><strong>Fix:</strong> Scope the timeout fork inside its own `fork...join` or `begin...end` block, or explicitly disable named blocks (e.g., `disable target_block_name;`).</p>
      </div>
    </details>
  </CardContent>
</Card>
...
- Next: [Procedural Flow Control](/curriculum/T1_Foundational/F2C_Procedural_Code_and_Flow_Control/flow-control)
