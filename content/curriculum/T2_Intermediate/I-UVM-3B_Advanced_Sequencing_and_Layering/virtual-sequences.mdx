...
`}
  explanationSteps={[
    ...
    { target: "5-6", title: "Sequencer Handles", explanation: "It contains handles to the real sequencers in the testbench, in this case, `m_pci_sequencer` and `m_eth_sequencer`. These handles are typically assigned directly in the environment's connect phase." },
...
**Configuration is Key:** The handles in the virtual sequencer (`m_pci_sequencer`, `m_eth_sequencer`) must be connected to the actual sequencer instances in the testbench environment. This is typically done directly in the environment's `connect_phase` by pointing the virtual sequencer's handles to the specific agent sequencers.
