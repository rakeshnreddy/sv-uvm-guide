...
>
  ...
  > **Try it hands-on:** Open the [Sequencer Arbitration Sandbox](/exercises/sequencer-arbitration) to queue requests, toggle lock/grab, and watch the rationale the sequencer prints for each grant. It mirrors the patterns in this lesson and is the quickest way to demo starvation fixes to teammates.
  ...
  **Why it matters**
  - Lines 5-19 show the body throttling itself until credits return, preventing overrun without blocking the driver.
  - Lines 21-29 demonstrate how `mid_do` can both change priority and veto the current grant, letting the sequencer immediately reconsider other requests.
  ...
  - Capture tricky scenarios with the sandbox mentioned above. Record GIFs for design reviews so the arbitration story stays visual.
  ...
  **A sequence needs five uninterrupted beats to program a mode register. What is the best tool?**
  - Increase its priority with `set_priority(10)`
  - Call `lock()` before the burst and `unlock()` after *(correct)*
  - Call `grab()` at the start of the test
  - Switch the sequencer to `SEQ_ARB_RANDOM`

  **You enabled `SEQ_ARB_WEIGHTED` but traffic is still uniform. What is missing?**
  - Override `mid_do`
  - Call `set_priority()` on each sequence to supply weights *(correct)*
  - Switch the driver to `try_next_item()`
  - Enable the command line processor
