...
>
  ...
  *IEEE 1800-2023, Clause 15.3: Semaphores*
  ...
  ## Failure Modes: Key Leaks and Deadlock

  **The Lost Key / Leak:** 
  The most common semaphore bug is acquiring a key and failing to return it. If a thread hits an `early return`, throws a fatal exception, or gets killed via `disable fork` before it calls `sem.put(...)`, that key is gone forever. Subsequent threads calling `get()` will hang, starving the system. To avoid this, always structure your loops so `put()` is guaranteed, or wrap critical sections carefully.

  **Deadlock (Circular Wait):**
  If Thread A holds Semaphore 1 and waits for Semaphore 2, while Thread B holds Semaphore 2 and waits for Semaphore 1, the simulation halts indefinitely. 
  
  **The Fix:** Always acquire multiple semaphores in a strict, globally agreed-upon order, or use `try_get` with timeouts and randomized backoffs to break the cycle.

  **Coordinating Random Stimulus:** When multiple threads generate randomized transactions, semaphores ensure only a safe number run concurrently. For more on stimulus generation, revisit [Constrained Randomization](/curriculum/T2_Intermediate/I-SV-2A_Constrained_Randomization_Fundamentals).
