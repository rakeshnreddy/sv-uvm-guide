...
<InteractiveCode>
```systemverilog
// A simple transaction class
class Transaction;
  rand bit [1:0] kind; // 0=READ, 1=WRITE, 2=IDLE, 3=RESET
  rand int       addr;
endclass

// Coverage collector component
class CoverageCollector;
  // Covergroup to be triggered on each new transaction
  covergroup CovGroup with function sample(Transaction tr);
    // Coverpoint for the 'kind' property
    option.per_instance = 1;
    cp_kind: coverpoint tr.kind {
      bins read_bin  = {0};
      bins write_bin = {1};
      bins other_bins[] = {[2:3]}; // Automatically create bins for 2 and 3
    }
    // Coverpoint for the 'addr' property
  ...
  function void sample_coverage(Transaction tr);
    this.CovGroup.sample(tr);
  endfunction
endclass

program main;
  initial begin
    CoverageCollector collector = new();
    Transaction       tr        = new();
    // Simulate receiving 50 transactions
    repeat (50) begin
      tr.randomize();
      $display("Sampling transaction: kind=%d, addr=%d", tr.kind, tr.addr);
      collector.sample_coverage(tr);
    end
  end
endprogram
```
