    ...
    questions={[
      {
        ...
        explanation: `Concurrent assertions run alongside the design and evaluate every sampling event, whereas immediate assertions are single-point checks.`
      },
      {
        ...
        explanation: `The delay \`##[1:5]\` widens the acceptance window so the acknowledgement may arrive in any of the next five samples.`
      }
    ]}
  />
  ...
  <div className="mt-8">
    <InterviewQuestionPlayground
      ...
      options={[
        ...
        { text: `Set 'type_option.weight = 0' on the properties.`, isCorrect: false, feedback: `That's also functional coverage, not an assertion construct.` }
      ]}
    />
  </div>
...
- Next: [`/curriculum/T2_Intermediate/I-SV-4B_Advanced_Temporal_Logic`](../I-SV-4B_Advanced_Temporal_Logic/).
