...
Sequencers manage high-level intent, drivers translate that intent into pin-wiggles. Their protocol keeps stimulus flowing:
1. Driver blocks on `seq_item_port.get_next_item(req)`.
2. A sequence running on the sequencer produces the transaction.
3. The driver drives the interface using the transaction fields.
4. `item_done()` tells the sequencer the driver is ready for the next request—skip it and the test hangs.
