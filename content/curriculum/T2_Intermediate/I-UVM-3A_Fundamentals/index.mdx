...
## Make It Work
1. **Define the Item:** Extend `uvm_sequence_item`, register fields, and add constraints.
2. **Define the Sequence:** Extend `uvm_sequence`, parameterize with the item, and write the `body()` task using `uvm_do` or `start_item`/`finish_item`.
3. **Start the Sequence:** Call `seq.start(sequencer)` from your test's run phase.

## Push Further
- Explore virtual sequences for coordinating multiple sequencers across a complex environment.
- Learn about the priority argument in `start()` for arbitration.

## Explore Nested Sequence Execution

Use the interactive explorer below to step through how nested sequences execute. Watch the `start()` → `body()` → `finish_item()` lifecycle events fire in order as the call tree is traversed depth-first.
