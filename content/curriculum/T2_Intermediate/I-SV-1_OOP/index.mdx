...
> *SystemVerilog OOP is formally defined in **IEEE 1800-2023, Clause 8: Classes**.*
...
- **Memory hook:** remember *Instantiate → Drive → Check*—now you’ll wrap each step in classes instead of raw tasks. If these ideas feel rusty, revisit the Tier-1 spotlight embedded in `F2A_Data_Types_and_Arrays` before continuing.
...
- [Constructors, `this`, and `super`](./constructors)
- [Parameterized and Abstract Classes](./parameterized-classes)
- **[Copying and Cloning Objects](./copying-and-cloning)** - *Interview Pitfalls*
- **[Polymorphism Pitfalls & Typesasting](./polymorphism-pitfalls)** - *Interview Pitfalls*
...
- Next: [`/curriculum/T2_Intermediate/I-SV-2A_Constrained_Randomization_Fundamentals`](../I-SV-2A_Constrained_Randomization_Fundamentals/).
...
<div className="mt-8">
  <InterviewQuestionPlayground
    question="A junior engineer forgot to declare the `pack()` method as `virtual` in the BasePacket class. What happens when a GenericDriver calls `pkt.pack()` on a BasePacket handle pointing to an EthernetPacket object?"
    options={[
      { text: "It fails to compile because drivers must use exact child handle types.", isCorrect: false, feedback: "Drivers almost always use base handle types for polymorphism." },
      { text: "The GenericDriver calls the BasePacket's `pack()` method, ignoring the child's implementation.", isCorrect: true, feedback: "Correct! Without `virtual`, method resolution is static (based on the handle type), not dynamic (based on the object type). This is a classic source of \"missing stimulus\" bugs." },
      { text: "The simulator throws a null-pointer exception.", isCorrect: false, feedback: "The method is executed, just the wrong one." },
      { text: "The EthernetPacket's `pack()` method is called correctly because inheritance automatically propagates method overrides.", isCorrect: false, feedback: "Only virtual methods propagate overrides to base class handles." }
    ]}
  />
</div>
