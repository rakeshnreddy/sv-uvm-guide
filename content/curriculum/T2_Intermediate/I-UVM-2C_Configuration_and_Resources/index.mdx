...
### The Database Mechanics (IEEE 1800.2-2020 Clause 23)
The UVM configuration DB is essentially a strongly-typed string-keyed dictionary that respects testbench hierarchy. It is built on top of the lower-level `uvm_resource_db`. While `config_db` enforces hierarchy precedence (a `uvm_env`'s `set()` overrides an `uvm_agent`'s `set()`), `resource_db` relies strictly on a chronological or explicit precedence value.

**Rule of Thumb:** Use `uvm_config_db` for 99% of settings. It automatically injects the hierarchical context string. Fall back to `resource_db` only if you need exotic globally-scoped precedence control that defies standard testbench hierarchy.
...
### Precedence and Path Matching
The UVM configuration database uses glob matching. The context parameter (`cntxt`) and the target string (`inst_name`) are concatenated to form a fully resolved path. 

When a component calls `get()`, UVM checks if that component's `get_full_name()` matches any of the stored `set()` paths. If multiple match, **the highest component in the hierarchy wins** (top-down precedence during the build phase).
However, if strings are set during the `run_phase` (runtime), **the latest set() call wins**.
...
**Late Runtime Overrides:**
If someone calls `set()` *after* the `build_phase` has finished and the target components have already called `get()`, the value will sit in the database unused. The `config_db` is not reactive; components must explicitly `get()` the value again to see runtime changes.

## Practice & Reinforce

> **Lab Exercise:** [Debug the Null VIF](/practice/lab/config-debug)
> 
> Put the theory into practice. Learn to trace hierarchy spelling typos to fix a fatal null virtual interface error caused by a sloppy `config_db` string.
