...
<a id="monitors-scoreboards-and-the-analysis-fabric"></a>

### The Analysis Fabric (Publish/Subscribe)
Monitors reconstruct transactions and broadcast them. They use a `uvm_analysis_port`, which expects listeners to provide a path to a terminal node that implements the analysis write method.

> **Correction on Hierarchy:** A common mistake is thinking an `analysis_port` connects directly to an `analysis_export`. It does *not*. In UVM, an `analysis_port` must ultimately resolve to an `analysis_imp` (implementation) which provides the actual `write()` SV function. The `analysis_export` is just a pass-through structural node used to cross hierarchical boundaries (like an Agent boundary).
...
## Decoupling the Scoreboard
A critical design pattern in UVM is decoupling the fast monitor from the slow scoreboard. Because `write()` is a blocking function call (it executes in zero time, but the thread cannot advance until `write()` finishes), a slow scoreboard will artificially prevent the monitor from capturing the next bus cycle!

To fix this, we insert a `uvm_tlm_analysis_fifo` between them:
1. The Monitor's `analysis_port` connects to the FIFO's `analysis_export`. 
2. The Scoreboard no longer implements `write()`. Instead, it runs a `forever` loop in its `run_phase`, calling `get()` on the FIFO's blocking `get_export`. 
3. Now the Monitor can spray transactions into the elastic FIFO buffer without being blocked by the Scoreboard's processing delay.

## Make It Work
1. **Connect everything in `connect_phase`**: Ports exist after the `build_phase` completes. Connect them before `run_phase` wakes up.
2. **Analysis Ports support 0-to-N connections**: You can wire an agent's `analysis_port` to an `analysis_imp` on a subscriber, a FIFO for a scoreboard, and a logger simultaneously.
3. **Use FIFOs for decoupling**: If your environment components operate on completely different clocks, place a `uvm_tlm_analysis_fifo` between them.
...
### Interactive: Build the TLM Topology

Use the interactive builder below to practice wiring UVM components together. Click a source port, then click a compatible destination to connect them. Use **Check Connections** to validate your work.

<TlmConnectionBuilderVisualizer />

<div className="my-6 rounded-2xl border border-muted bg-muted/40 p-6">
  <h3 className="text-lg font-semibold">Lab: Wire It Up!</h3>
  <p className="mt-2 text-sm text-muted-foreground">
    In this lab, practice connecting the `seq_item_port` to the `seq_item_export`. 
    Notice how transactions flow from the sequence to the driver. 
  </p>
...
<div className="my-6 rounded-2xl border border-muted bg-muted/40 p-6">
  <h3 className="text-lg font-semibold">Lab: Decoupling the Scoreboard</h3>
  <p className="mt-2 text-sm text-muted-foreground">
    Put the theory into practice. Learn to diagnose artificial monitor backpressure, instantiate an analysis FIFO, and refactor a slow Scoreboard to use a blocking `get()` loop instead of a `write()` implementation.
  </p>
  <div className="mt-4">
    <a href="/practice/lab/scoreboard-decoupling" className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow ring-offset-background transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
      Start the Scoreboard Lab →
    </a>
  </div>
</div>
