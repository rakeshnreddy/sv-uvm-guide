...
>
  ...
  *IEEE 1800-2023, Clause 19.7.1: Predefined coverage options*
...
Coverage collectors often subscribe to monitor analysis ports. See [analysis components](/curriculum/T2_Intermediate/I-UVM-2B_TLM_Connections#monitors-scoreboards-and-the-analysis-fabric) for how transactions reach these covergroups.

  Reports can then filter coverage by `option.comment`, giving traceability back to the V-Plan. When holes appear, adjust stimulus using [Constrained Randomization](/curriculum/T2_Intermediate/I-SV-2A_Constrained_Randomization_Fundamentals).
  ...
  **Granularity:** Tag the bin level when requirements are nuanced—e.g., `illegal_bins` for forbidden modes vs. `bins` for required ones. Traceability works best when the V-Plan mirrors this structure.
  ...
  **Process guardrails:** Schedule periodic coverage reviews with the plan owners. Any time a requirement changes, update both the plan YAML and the covergroup options together so the linkage never drifts.
  ...
  <div className="mt-8">
    <InterviewQuestionPlayground
      question="A verification engineer has 10 instances of `cg_mode`, representing 10 different channels. They want to ensure they see in the final report which specific channel missed a coverage bin. Which setup step is critical to make this report readable?"
      options={[
        { text: "They must set type_option.merge_instances = 1.", isCorrect: false, feedback: "Merging instances destroys visibility into *which* specific channel missed the bin." },
        { text: "They must call cg_mode.set_inst_name() with a unique string for each instance.", isCorrect: true, feedback: "Correct! Without calling `set_inst_name()`, all instances might just appear as `cg_mode` in the database, making it impossible to know which of the 10 channels failed." },
        { text: "They need to use cross-coverage instead of option.per_instance.", isCorrect: false, feedback: "Cross coverage links features, not instances." },
        { text: "They must set option.goal = 100 for every instance.", isCorrect: false, feedback: "Goal setting doesn't help identify which instance is failing." }
      ]}
    />
  </div>
