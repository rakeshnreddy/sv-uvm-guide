...
<a id="sequencer-and-driver-handshake" />
...
1. The driver parks in `get_next_item(req)` and waits for the sequencer to produce work.
2. A sequence requests access with `start_item(req)`, wins arbitration, and finalizes the transaction with `finish_item(req)` (the macros hide these steps inside `uvm_do`).
3. The sequencer releases the item, letting `get_next_item()` return.
4. The driver executes `drive_transfer(req)` on the interface.
5. `item_done()` (optionally with a response) unblocks the sequencer so the next transaction can flow.
...
### Try Sequencer Arbitration Decisions

Extend the mental model by experimenting with locks, grabs, and arbitration modes. Add requests for each sequence, toggle `lock()`/`unlock()` or `grab()` and see how the sequencer responds.

<div className="my-6 rounded-2xl border border-muted bg-muted/40 p-6">
  <h3 className="text-lg font-semibold">Hands-On Sequencer Arbitration</h3>
  <p className="mt-2 text-sm text-muted-foreground">
    Jump into the interactive sandbox under
    <code className="mx-1">Practice → Exercises → Sequencer Arbitration</code> to experiment with
    locks, grabs, and different arbitration modes. It mirrors the scenarios described here and lets
    you see the driver logs update in real time.
  </p>
</div>
...
- [ ] You can sketch the handshake timeline from memory.
...
- Sandbox – Recreate tricky arbitration scenarios in the [sequencer sandbox](/exercises/sequencer-arbitration) to see how locks and grabs interact.
